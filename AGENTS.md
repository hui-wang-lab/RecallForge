...
- 默认 RAG 形态：Agentic RAG，由 Agent 按需调用受控检索工具搜索知识库。
...
| LLM       | 由 `AgentConfig` 注入 | 上下文 ≥ 32k | 不在代码中硬编码厂商 |
...
Agent 默认启用知识库搜索，但**不允许直接持有任何 VectorDB 句柄**。检索能力必须通过工具暴露：

```python
agent = Agent(
    name="RAG Assistant",
    tools=[search_internal_kb],   # 包装 retrieval 服务的工具
    markdown=True,
    instructions=[
        "需要事实依据的回答必须先调用 search_internal_kb。",
        "如果知识库没有足够依据，明确说明无法从当前资料确认。",
        "不要编造文档中不存在的政策、价格、日期或接口行为。",
        "回答必须保留 search_internal_kb 返回的引用信息。",
    ],
)
```
...
1. **禁止**直接使用 `agno.knowledge.Knowledge` 默认 VectorDB（例如 `agno.vectordb.pgvector.PgVector`）。
2. **禁止**把 `VectorStoreAdapter` 直接交给 Agno 框架自管理。
3. 检索能力必须包装为 Agno `Tool`，例如 `search_internal_kb(query: str, filters: KbFilters)`，工具内部完整走：
   ...
   - parent chunk 回查
   - references 组装
4. 如果未来确实需要使用 `agno.knowledge` 形态，必须实现一个自定义 `VectorDb` 适配器，**内部委托给 `VectorStoreAdapter`**，且必须在该适配器中强制注入权限上下文，禁止任何"直连数据库"路径。

#### 请求上下文与权限传递

Agent 工具调用是 LLM 自主驱动的多轮过程，必须保证工具执行时能拿到当前请求的权限上下文，且 LLM 无法伪造：

- 定义 `RequestContext(tenant_id, user_id, department, access_level, request_id)`，在 API 入口创建并通过 `contextvars.ContextVar` 注入到执行作用域。
- 工具实现内部从 `ContextVar` 读取 `RequestContext`，构造服务端 metadata filter。
- 工具的 JSON schema **不得**暴露 `tenant_id`、`user_id`、`department`、`access_level` 等字段给 LLM。LLM 只能传业务维度的 `filters`（例如 `doc_type`、`source_uri`、`date_range`）。
- 服务端在调用 `VectorStoreAdapter.search()` 之前对 LLM 传入的 filters 做白名单校验，发现越权字段直接抛错并写入审计日志。
- 必须有单元测试覆盖："LLM 在 filters 中传入 `tenant_id='*'` 或试图越权时，调用被拒绝且不留下任何召回痕迹"。

Agent 本身不负责判断权限边界。权限过滤一律在服务端和检索层完成。
