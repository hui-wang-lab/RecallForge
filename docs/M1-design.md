...
- `rag_chunks.embedding_model`、`rag_chunks.embedding_provider`、`rag_chunks.embedding_dim` 记录 M1 基线 embedding 描述；多模型扩展时，新增向量列必须同步更新 repository 的 `embedding_model -> column + dim + metric` 映射。
...
| `department` | `TEXT` | nullable | 服务端权限字段 |
| `access_level` | `TEXT` | not null | 服务端权限字段 |
...
- `UNIQUE (tenant_id, source_uri, content_hash)`，避免历史重复内容污染 chunk。
...
- `(tenant_id, document_id, status)`、`(tenant_id, status, version)` 索引。
...
| `department` | `TEXT` | nullable | 服务端权限过滤字段 |
| `access_level` | `TEXT` | not null | 服务端权限过滤字段 |
...
| `status` | `TEXT` | not null | `active`、`superseded`、`deleted` |
| `embedding_provider` | `TEXT` | not null | 基线 provider，例如 `dashscope` |
| `embedding_model` | `TEXT` | not null | 基线模型，例如 `text-embedding-v4@1024` |
| `embedding_dim` | `INTEGER` | not null | 基线维度，例如 `1024` |
...
- 基线向量列使用 cosine HNSW 索引；小数据量阶段可以通过配置延迟创建或不使用该索引，但 DDL 预览保留索引形态。
...
| `tenant_id` | `TEXT` | not null | 租户 |
| `user_id` | `TEXT` | not null | 审计身份，不进入 chunk metadata |
| `department` | `TEXT` | nullable | 请求权限上下文 |
| `access_level` | `TEXT` | not null | 请求权限上下文 |
...
CREATE TABLE rag_documents (
    ...
    CONSTRAINT uq_rag_documents_source_version
        UNIQUE (tenant_id, source_uri, version),
    CONSTRAINT uq_rag_documents_source_hash
...
get(document_id: DocumentId, tenant_id: TenantId) -> DocumentRecord | None
get_latest_by_source(tenant_id: TenantId, source_uri: str) -> DocumentRecord | None
find_by_source_hash(tenant_id: TenantId, source_uri: str, content_hash: str) -> DocumentRecord | None
next_version(tenant_id: TenantId, source_uri: str) -> int
mark_source_superseded(tenant_id: TenantId, source_uri: str, except_document_id: DocumentId | None = None) -> int
...
get(parent_id: ParentChunkId, tenant_id: TenantId) -> ParentChunkRecord | None
get_by_document_and_key(document_id: DocumentId, parent_key: str) -> ParentChunkRecord | None
get_by_ids(tenant_id: TenantId, parent_ids: Sequence[ParentChunkId]) -> list[ParentChunkRecord]
mark_by_document_status(document_id: DocumentId, tenant_id: TenantId, status: Literal["active", "superseded", "deleted"]) -> int
```
...
get(chunk_id: ChunkId, tenant_id: TenantId) -> ChildChunkRecord | None
get_by_ids(tenant_id: TenantId, chunk_ids: Sequence[ChunkId]) -> list[ChildChunkRecord]
get_by_parent_id(tenant_id: TenantId, parent_id: ParentChunkId, status: str = "active") -> list[ChildChunkRecord]
list_for_embedding_backfill(embedding_model: str, limit: int, tenant_id: TenantId | None = None) -> list[ChildChunkEmbeddingSource]
mark_by_document_status(document_id: DocumentId, tenant_id: TenantId, status: Literal["active", "superseded", "deleted"]) -> int
...
2. 计算规范化内容 hash。
3. 通过 `DocumentRepository.find_by_source_hash()` 判断重复导入。
4. hash 命中时，不新增 document/chunk，任务状态写成 `skipped_duplicate`。
5. hash 不命中时，通过 `next_version()` 创建新版本 document。
6. 同一 `(tenant_id, source_uri)` 的旧 active document、parent chunk、child chunk 标记为 `superseded`。
7. `package.parent_chunks` 写入 `rag_parent_chunks`。
8. `package.child_chunks` 写入 `rag_chunks`，embedding 列保持 `NULL`。
9. `parse_report`、`warnings`、`parser_used`、`chunker_used` 回写 `rag_ingest_jobs`。
...
2. 启动时校验 `embedding_text_embedding_v4_1024` 的维度与 `EmbeddingProvider.dim` 一致。
...
5. `VectorStoreAdapter.upsert_chunks()` 通过 `PgVectorStore` 写入对应向量列。
...
- hash 来源是规范化文档内容，不使用原始文件路径或上传时间。
- 同一 `(tenant_id, source_uri, content_hash)` 命中时，导入任务写 `skipped_duplicate`。
...
- hash 变化时创建新版本，并将旧 active 版本标记为 `superseded`。
- 默认检索只命中 active 最新版本。
